...
CI/CD 방식의 지속적 통합 유지 할 수 있도록 빌드/설계/테스트/패키지/배포의 단계로 수행 할 수 있도록 설계 되었으며, 
Gitlab API를 이용하여 릴리즈 노트 생성 자동화 등의 기능도 추가로 지원합니다.

배포는 Kustomization 방식의 배포 정책을 따르도록 구성되었으며, 
ArgoCD에 의 배포 되도록 준비하였습니다. 
사내 배포 문화 정착을 위해 브랜치 전략과 가이드를 전파하였습니다.

주요기술 #Jenkins Blueocean #Kustomize #Argocd

### 프로모션

프로모션은 이벤트 또는 프로모션 기간 동안 안내를 위한 웹뷰 정보를 관리하는 서비스 입니다.
...
##### 개발
프로모션에 결재 기능을 연동하기 위해 구매 이력을 확인하여 필터링 하는 기능과 
웹뷰의 노출 기간을 처리 할 수 있는 기능을 개발하였습니다. 
...
- 2021.02 ~ NOW
##### 개발
현재 운영되고 있는 프로젝트를 운영 효율성을 높이기 위해 Spring Webflux를 이용하여 
...
게임 마다 다른 데이터를 표준화하고, 
각 게임사에 API를 통한 데이터 동기화가 핵심인 서비스이며, 
게임 서버(채널) 통합 등의 업데이트 작업 시 데이터 마이그레이션 지원 운영을 하였습니다.

#### 프로젝트 : 프로필 마이그레이션 툴 개발
##### 기간
2020.01 ~ 2020.03
##### 개발
블레이드앤소울 월드 통합으로 인해 월드별로 구분된 프로필 데이터 통합에 대한 요구사항이 있었습니다. 
이 외에도 데이터 통합 및 초기화에 대한 요구사항이 지속적으로 있어왔고, 
...
2020.02 ~ 2020.04
##### 개발
CSV 파일 업로드 방식을 사내에 GSuit이 도입되어 있기 때문에 구글 파일 드라이브의 스프레드시트를 이용하여 업무 효율화하였습니다. 
업로드 API 및 스프레드시트 불러오기, 구글 OAuth 인증 기능을 내부 관리툴에서 모두 적용 할 수 있도록 파일 업로드 API를 래핑한 
사내 라이브러리 개발하였고, 사내 관리 툴에 적용 될 수 있도록 가이드를 하였습니다.

주요기술 #Aerospike #Spring Boot #Google File Drive #Apache Storm


### 지표
2020.02 ~ NOW

<샘플 자료입니다. 회사와는 관계 없습니다>

클라이언트/서버 로그 기반 플랫폼 공통 지표를 관리했습니다.
Spark를 통해 Dimension, Metric을 분리하여 데이터를 관리했고, 운영 중 사용자 정의 지표를 요구 할 경우 
Google Bigdata로 데이터를 이관하여 처리하였습니다. 현재 플랫폼 지표 운영 개선을 위해 
스트림/배치 데이터 구분과 데이터마트 구축 방안을 제안하여 진행 계획 중에 있습니다.

주요기술 #Spring Boot #Crate.io #Apache Spark #Google Bigtable #Kafka #Grafana
